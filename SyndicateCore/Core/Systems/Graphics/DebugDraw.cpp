...
struct GLRenderPoints
{
    ...
    void Flush()
	{
		...
		//glUniformMatrix4fv(m_projectionUniform, 1, GL_FALSE, g_debugDraw.getPrMatrix().elements);
...
void GLRenderLines::Create(const glm::mat4& prMatrix)
{
...
void GLRenderLines::Vertex(const b2Vec2& v, const b2Color& c)
{
	if (m_count == e_maxVertices)
		Flush();
...
void GLRenderLines::Flush()
{
	...
	glUniformMatrix4fv(m_projectionUniform, 1, GL_FALSE, &this->m_PrMatrix[0][0]);
...
void GLRenderTriangles::Create(const glm::mat4& prMatrix)
{
	this->m_PrMatrix = prMatrix;
...
void DebugDraw::Create()
{
	...
	m_lines->Create(this->m_PrMatrix);
	m_triangles = new GLRenderTriangles();
	m_triangles->Create(this->m_PrMatrix);
}
